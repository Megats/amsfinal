...
public class CartDAO {
	...
	public static void addCart(Cart bean) {
		...
		try {
			...
			ps = con.prepareStatement(
					"insert into orderdetail (totalprice,quantity,orderid,productid) values (?,?,?,?)");
	...
	public static List<Cart> getOrderById(String orderid) {
		...
		try {
			...
			// String query = "select * from person where id=?";
			ps = con.prepareStatement(
					"select productid,productname,productprice,quantity from product join orderdetail using (productid) join orders using (orderid) where orderid=?");
